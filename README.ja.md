...
観点、日本市場向けの翻訳ガイダンス、候補数/採用数の制限を含めます。これにより、
並列で動く Codex research thread が無制約な検索プロンプトになることを防ぎつつ、
探索量も確保します。既定では、Web検索が有効な場合に各トラックで少なくとも15件の
候補を確認し、最終的に最大8件のsignalを返すよう指示します。
